...
function ResultCard({ data }) {
  ...
  // ✅ UPDATED: real trend
  const trend = [
    { time: "Morning", temp: weather.trend?.morning },
    { time: "Afternoon", temp: weather.trend?.afternoon },
    { time: "Evening", temp: weather.trend?.evening },
  ];
  ...
  return (
    <div className="mt-10 max-w-md mx-auto px-4">
      <div className="backdrop-blur-xl bg-white/10 border border-white/10 rounded-2xl shadow-2xl p-6 text-white">
        ...
        {/* ✅ NEW DAILY PLAN */}
        <div className="mt-5 bg-green-500/10 border border-green-500/20 p-3 rounded-xl">
          <p className="text-sm text-green-400 text-center">📅 Daily Plan</p>
          <p className="text-xs text-green-300 mt-2">🌅 Morning → {ai.plan?.morning || "-"}</p>
          <p className="text-xs text-green-300">☀️ Afternoon → {ai.plan?.afternoon || "-"}</p>
          <p className="text-xs text-green-300">🌆 Evening → {ai.plan?.evening || "-"}</p>
        </div>
